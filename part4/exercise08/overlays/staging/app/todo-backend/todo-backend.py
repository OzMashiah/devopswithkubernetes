...
async def publish_task(task_data):
    """Publish the task update to NATS."""
    await nc.connect(NATS_URL)
    await nc.publish(NATS_SUBJECT, json.dumps(task_data).encode())
...
def create_todo():
    ...
    # Publish to NATS about the new to-do
    task_data = {
        "id": new_id,
        "todo": todo_item,
        "done": False
...
def update_todo_status(id):
    ...
    if updated_todo:
        # Publish to NATS about the updated to-do
        task_data = {
            "id": updated_todo[0],
            "todo": updated_todo[1],
            "done": updated_todo[2]
